...
class Cabin:
    ...
    def _build_base(self):
        base = ""
        for y_coordinate in range(self.heigth - self.height_width_from_middle):
            for x_coordinate in range(self.width):
                if ((x_coordinate == self.middle_of_the_house - self.height_width_from_middle   # left wall
                     or x_coordinate == self.middle_of_the_house + self.height_width_from_middle)   # right wall
                        or ((y_coordinate + self.height_width_from_middle + 1 == self.heigth
                             or y_coordinate == 0)
                            and (1 < x_coordinate < self.width - 2))):
                    base += self.wall_brick
                    continue
                if y_coordinate == self.heigth - self.height_of_fence - self.height_width_from_middle:     # horizontal fence
                    base += self.horizontal_fence
                    continue
                if y_coordinate > self.heigth - self.height_of_fence - self.height_width_from_middle and x_coordinate % 2 == 0:     # vertical fence
